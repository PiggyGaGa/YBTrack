...
DATA_ROOT = '/media/rjzx/STORE/users/gyd7409/VisDrone2019'
...
def generate_imgs_and_labels(opts):
    ...
    # gyd 处理 pedestrain-1 people-2 归为一类   
    # bycle-3 单独一类
    # car-4, van-5, truck-6, bus-9单独一类
    # tricycle-7, own-tricycle-8 归为一类
    # motor-10 单独归为一类
    # 总共有5类，这样把原来的类别区分开
    category_dict = {1:0, 2:0, 3:1, 4:2, 5:2, 6:2, 7:3, 8:3, 9:2, 10:4}
        ...
        if write_txt:
            ...
            with open(to_file, 'a') as f:
                for idx, img in enumerate(imgs):
                    ...
                    if exist_gts[idx]:
                        f.write(DATA_ROOT + '/' + 'images/' + opts.split_name + '/' \
                                + seq + '/' + img + '\n')
