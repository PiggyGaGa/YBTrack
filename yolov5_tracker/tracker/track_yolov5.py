...
def main(opts, cfgs):
    ...
    if opts.data_format == 'yolo':
        with open(f'{opts.dataset_dir}/{opts.dataset}/test.txt', 'r') as f:
            lines = f.readlines()
            for line in lines:
    ...
    for seq in seqs:
        ...
        path = os.path.join(DATA_ROOT, seq) if opts.data_format == 'origin' else os.path.join(opts.dataset_dir, f'{opts.dataset}', 'test.txt')
    ...
    #     dataset_list = [trackeval.datasets.MotChallenge2DBox(dataset_config)] if opts.dataset in ['mot', 'mot17', 'uavdt'] else [trackeval.datasets.VisDrone2DBox(dataset_config)]
    #     metrics_list = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_dir', type=str, default='./', help='dataset dir which contains test.txt')
