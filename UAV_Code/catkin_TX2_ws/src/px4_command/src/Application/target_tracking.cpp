...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>全 局 变 量<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
px4_command::Diff read_pixel;
px4_command::gimbal read_gimbal;
enum Command
{
    Idle,
    Takeoff,
...
float flag_detected = 0.0;                                          // 是否检测到目标标志
//---------------------------------------Track---------------------------------------------
...
*/
void sub_diff_callback(const px4_command::Diff::ConstPtr& msg)
{ 
   read_pixel.recsize = msg->recsize;
   read_pixel.selectrec = msg->selectrec;
}

void sub_gimbaldata_cb(const px4_command::gimbal::ConstPtr& msg)
{ 
...
void sub_diff_flag_cb(const geometry_msgs::Point::ConstPtr& msg)
{
   flag_detected = msg->x;
//cout<<"@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"<<endl;
}
...
int main(int argc, char **argv)
{
    ...
    //vision_pixel
    ros::Subscriber pos_diff_sub = nh.subscribe<px4_command::Diff>("/position_diff", 10, sub_diff_callback);
    //视觉flag
    ros::Subscriber pos_diff_flag_sub = nh.subscribe<geometry_msgs::Point>("/relative_position_flag", 10, sub_diff_flag_cb);
    //gimbal_data
    ros::Subscriber gimbaldata_sub = nh.subscribe<px4_command::gimbal>("/readgimbal", 10, sub_gimbaldata_cb);
...
*/
                ...
                else
                {
                    ...
                    comid++;
                        ...
                        Command_now.vel_sp[0] =  4 * kpx_track * (1-read_pixel.recsize/read_pixel.selectrec);
                    Command_now.vel_sp[1] =  0;

                    //Height is locked.
                    //Command_now.vel_sp[2] =  - kpz_track * pos_target.position.z;
float pitch_angle = read_gimbal.imu1;

		    if (pitch_angle > 80 || pitch_angle < -20)
		    {
		     height_time = 0;
		     Command_now.pos_sp[2] = 0; 
                    ...
                    {
                     height_time += 0.06;
                    Command_now.pos_sp[2] = height_time;
                     }
                    ...
                    //目前航向角锁定
                    Command_now.yaw_sp = -3.141592653 * read_gimbal.rel2/180;
                   ...
                   // Command_now.vel_sp[2] = satfunc(Command_now.vel_sp[2], track_max_vel_z, track_thres_vel_z);
                    ...
                    //如果期望速度为0,则直接执行悬停指令
                    if(Command_now.vel_sp[0]==0 && Command_now.vel_sp[1] == 0 && abs(read_gimbal.rel2)< 3 )
                    {
                        Command_now.command = Hold;
                    }
